...
function initAdditionalCharts() {
    ...
    if (onboardingCtx) {
        ...
        // Also compute riding actuals for each month
        const ridingActuals = months.map((m, i) =>
            dataManager.corporateAssociates.reduce((sum, associate) =>
                sum + (dataManager.corporateRiding[associate]?.[i] || 0), 0)
        );
        new Chart(onboardingCtx.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [
                    ...
                    { label: 'Onboarded', data: onboardingActuals, backgroundColor: '#2A9D8F', borderColor: '#2A9D8F', borderWidth: 1 },
                    { label: 'Riding', data: ridingActuals, backgroundColor: '#F4A261', borderColor: '#E76F51', borderWidth: 1 }
                ]
            },
            options: {
                responsive: true,
    ...
    // Onboarded vs Riding (in Team Performance section)
...
function generateMonthlyReport() {
    let csv = 'Monthly Performance Report\n\nMonth,Target Revenue,Actual Revenue,Achievement %,Last Year Revenue,Growth %,Onboarding Target,Onboarding Actual,Onboarding Achievement %,Total Riding\n';
    ...
    months.forEach((month, i) => {
        ...
        const onboardingActual = dataManager.corporateAssociates.reduce((sum, associate) =>
            sum + (dataManager.corporateOnboarded[associate]?.[i] || 0), 0);
        const ridingActual = dataManager.corporateAssociates.reduce((sum, associate) =>
            sum + (dataManager.corporateRiding[associate]?.[i] || 0), 0);
        csv += `${month},${revenueTarget},${revenueActual},${revenueAchievement.toFixed(2)}%,${lastYear},${growth.toFixed(2)}%,84,${onboardingActual},${(84 > 0 ? (onboardingActual/84)*100 : 0).toFixed(2)}%,${ridingActual}\n`;
    });
    return csv;
}
function generateAssociateReport() {
    let csv = 'Associate Performance Report\n\nAssociate,Total Revenue,Net Revenue (18%),Monthly Avg Revenue,Total Onboarded,Total Riding,Riding Rate,Monthly Avg Onboarded,Revenue Achievement %,Status\n';
    Object.values(dataManager.overviewData.associatePerformance).forEach(associate => {
        ...
        const netRevenue = associate.totalRevenue * 0.18;
        csv += `${associate.name},${associate.totalRevenue},${netRevenue.toFixed(0)},${monthlyAvgRevenue.toFixed(0)},${associate.totalOnboarded},${associate.totalRiding || 0},${(associate.ridingRate || 0).toFixed(1)}%,${monthlyAvgOnboarded.toFixed(1)},${revenueAchievement.toFixed(2)}%,${status}\n`;
    });
    return csv;
}